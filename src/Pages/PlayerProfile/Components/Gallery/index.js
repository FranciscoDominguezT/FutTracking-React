...
const Gallery = ({ usuarioId }) => {
  ...
  useEffect(() => {
    if (usuarioId) {
      console.log("Obteniendo videos para usuarioId:", usuarioId);
      const fetchVideos = async () => {
        try {
            const response = await fetch(`${API_BASE_URL}/userProfile/videos/${usuarioId}`, {
            headers: {
  ...
  useEffect(() => {
    console.log("usuarioId recibido en Gallery:", usuarioId);
    if (usuarioId) {
      const fetchProfile = async () => {
        try {
          const response = await axios.get(`${API_BASE_URL}/profile/player/${usuarioId}`);
  ...
  const handleFollowToggle = async () => {
    ...
    try {
      const response = await axios.post(`${API_BASE_URL}/videos/${usuarioId}/${selectedVideo.usuarioid}/followChange`);
  ...
  useEffect(() => {
    const fetchFollow = async () => {
      try {
        const response = await axios.get(`${API_BASE_URL}/videos/${usuarioId}/${selectedVideo.usuarioid}/follow`);
        setIsFollowing(response.data.isFollowing);
      } catch (error) {
