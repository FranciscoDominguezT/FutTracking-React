...
const MailReclutador = () => {
  ...
  const handleConfirm = async () => {
    try {
      ...
      if (response.data.rol === 'Aficionado') {
        navigate('/reclutador', { state: { email, password } }); // Pasamos email y contraseña como state
      }
    } catch (error) {
      if (error.response) {
  ...
  return (
    <div className="register-container">
      ...
      <h1 className="change-rol">Reclutador</h1>
